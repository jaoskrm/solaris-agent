...
function ChatPanel({
    ...
    return (
        <Card className={`flex flex-col h-full border-${color}-900/30 bg-[#0c0c0c]/90`}>
            {/* Header */}
            <div className={`flex items-center justify-between p-4 border-b border-${color}-900/30`}>
            ...
            {/* Messages */}
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[400px] max-h-[500px]">
                {messages.map((msg) => (
                    <motion.div
                        key={msg.id}
                    ...
                    >
                        <div className={`max-w-[90%] ${msg.isUser ? 'bg-gray-800' : `bg-${color}-950/30 border border-${color}-900/20`} rounded-lg p-3`}>
                            {!msg.isUser && (
            ...
            <div className="p-4 border-t border-gray-800">
                <div className="flex gap-2">
                    <input
                        ...
                        disabled={isLoading}
                        className="flex-1 bg-gray-900/50 border border-gray-800 rounded-lg px-4 py-3 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-gray-600 transition-colors"
                    />
                    <button
                        onClick={handleSend}
                        disabled={!inputValue.trim() || isLoading}
                        className={`p-3 bg-${color}-600 hover:bg-${color}-500 disabled:bg-gray-800 disabled:text-gray-500 rounded-lg transition-colors`}
                    >
    ...
    const handleSendMessage = async (team: 'red' | 'blue', content: string) => {
        ...
        try {
            ...
            const history: ApiChatMessage[] = [
                { role: 'system', content: `You are a ${team} team security agent. You help analyze code security and provide actionable insights.` },
                { role: 'user', content }
            ];

            // Call the chat API
            const response = await sendChatMessage({ messages: history });

            // Add AI response
            const aiMessage: Message = {
                id: (Date.now() + 1).toString(),
                team,
                agent: team === 'red' ? 'Commander' : 'Analyst',
    ...
    return (
        <div className="w-full relative z-10 px-6 py-8 max-w-[1400px] mx-auto min-h-screen">
            ...
            <motion.div
                ...
                className="mb-8"
            >
                <h1 className="text-3xl font-bold tracking-tight text-white mb-1">Team Chat</h1>
                <p className="text-sm text-gray-400">Communicate with Red Team and Blue Team agents</p>
            </motion.div>
