...
def print_message(msg: A2AMessage):
    ...
    print(f"  Priority: {msg.priority.value if hasattr(msg.priority, 'value') else msg.priority}")
    print(f"{'─' * 60}")
...
async def run_mission(objective: str, target: str, max_iterations: int = 5):
    ...
    # Initialize sandbox
    print(f"{COLORS['system']}  ⚙ Initializing sandbox...{COLORS['reset']}")
    try:
        ...
        print(f"{COLORS['system']}  ⚠ Sandbox unavailable: {e}{COLORS['reset']}")
    ...
    try:
        ...
        print(f"{COLORS['system']}  ✅ Sandbox ready{COLORS['reset']}")
    except Exception as e:
        logger.error("Sandbox container creation failed: %s", e)
        print(f"{COLORS['system']}  ⚠ Sandbox container failed: {e}{COLORS['reset']}")
